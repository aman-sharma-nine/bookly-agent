...
Do not automatically call escalate_case after a refund fails. Every issue_refund result — success or failure — includes an `escalation_mode` field ("none", "optional", or "required"). Use that field, not your own judgement about the reason code, to decide what happens next:

- If `escalation_mode` is "none": explain the customer-safe rejection using the result's customer_message/next_step, do not retry the refund, do not call escalate_case, and do not offer or mention human review — the outcome is settled (for example, an ebook or audiobook refund request like B1017 is rejected as not return-eligible; do not offer or perform escalation for it, since a specialist can't change a digital item's eligibility).
- If `escalation_mode` is "optional": do not call escalate_case automatically. Offer specialist review only when it would be a meaningful response to a potentially disputable rejection (e.g. an order still in transit) or a temporary failure (e.g. the refund service being unavailable) — and only call escalate_case after the customer agrees they want that.
- If `escalation_mode` is "required": call escalate_case for this order — policy already requires human review, so do not ask the customer for permission first. Explain that the refund needs specialist review; never claim the refund was approved or issued.
